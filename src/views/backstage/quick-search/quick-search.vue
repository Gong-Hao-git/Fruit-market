<template>
  <div class="quick-search">
    <tableModel
      ...
      :tableList="tableList"
    />
    <quick-search-add ref="quickSearchAddRef" @refresh="refresh" />
  </div>
</template>
...
import { quickPage, quickDelete } from "@/api/backstage";
import quickSearchAdd from "./components/quick-search-add.vue";

let quickSearchAddRef = ref();
...
//查询和删除的接口
let fucData = {
  seach: quickPage, //查询的方法
  delete: quickDelete, //删除的方法，若没有此操作，可以不传
};
...
let titleButton = [
  ...
  },
  {
    ...
    icon: "icon-xinzeng",
    fuc: () => {
      quickSearchAddRef.value.setDialog(true);
    },
  },
];
    ...
    children: [
      {
        ...
        fuc: (data: any) => {
          //为其他操作时，把要执行的方法放在这儿
          let id = data.id;
          quickSearchAddRef.value.setDialog(true, id);
        },
      },
      {
